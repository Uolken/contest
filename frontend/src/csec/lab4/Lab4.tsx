...
export default () => {
  ...
  const [signature, setSignature] = useState<FileList | null>(null)
  const [signatureConfirmed, setSignatureConfirmed] = useState<boolean| undefined>(undefined)
  const [keys, setKeys] = useState<{ private: string, public: string } | null>(null);

  return <div className={styles.page}>
    <div className={styles.form}>
      <Button action={() => {
        axios.get("http://localhost:8080/csec4/generate")
      ...
      <div className={styles.fileButtons}>
        <Button action={() => {
          ...
          axios.post("http://localhost:8080/csec4/sign", data, {
            responseType: 'blob'
          })
          .then(r => {
              ...
              const link = document.createElement('a')
              link.href = url
              link.setAttribute('download', file?.item(0)!.name + ".signed") //or any other extension
              document.body.appendChild(link)
              link.click()
            }
          )
      ...
      <label>
        Выберите подпись:
        <input type="file" onChange={e => setSignature(e.target.files)}/>
      </label>
      <div className={styles.fileButtons}>
        <Button action={() => {
          const data = new FormData()
          file?.item(0) && data.append("file", file.item(0)!)
          signature?.item(0) && data.append("signature", signature.item(0)!)
          data.append("publicKey", keys!.public)
          axios.post("http://localhost:8080/csec4/verify", data)
          .then(r => {setSignatureConfirmed(r.data.verified)})
        }} text={"Проверить подпись"}/>
      </div>

      {signatureConfirmed === true && <div>Подпись подтверждена</div>}
      {signatureConfirmed === false && <div>Подпись не подтверждена</div>}
    </div>
